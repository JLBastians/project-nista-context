...
| **Frontend hosting** | Vercel. Main branch = landing page (strobelab.co); dev branch = web app (running on mock data, not yet connected to backend). | April 2026 | Deep Next.js integration. Environment variables for API endpoints. Preview deployments. Vercel Analytics included. Zero-config deployment. |
...
| **Product primary key strategy** | Keep integer SERIAL PKs. Cast to string (`str(product_id)`) at the API response layer. No UUID migration. | April 2026 | Integer PKs are simpler and already in use across scrapers and enrichment pipeline. String cast at the Pydantic layer is a one-line change and keeps backward compatibility. UUID migration would require updating all existing rows, scrapers, and tooling. |
